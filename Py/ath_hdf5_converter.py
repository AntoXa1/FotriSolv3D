...
##  python plot_spherical.py atorus.out2.00075.athdf rho show
## python plot_spherical.py atorus.out2.00000.athdf rho --stream Bcc  show
...
# ('kwargs=0', {'stream_samples': 100, 'logr': False, 'colormap': None, 'vmin': None, 'level': None, 'data_file': 'atorus.out2.00075.athdf', 'average': False, 'output_file': 'show', 'stream_average': False, 'stream_density': 1.0, 'r_max': None, 'logc': False, 'vmax': None, 'stream': None, 'theta_compression': None, 'midplane': False, 'quantity': 'rho'}, 'atorus.out2.00075.athdf')
...
# Athena++ modules
# import athena_read_mod as athena_read
import athena_read

# print(" ----------------- converter script -----------------" )
# level = None
# h = 1
# quantities = ["rho"]
# data = athena_read.athdf('../average_08483.athdf', quantities=quantities,
#                                      level=level)
# print(data)
# exit()
...
def main(**kwargs):
    ...
    if kwargs['theta_compression'] is not None:
        ...
        if quantities[0] == 'Levels':
            ...
            data = athena_read.athdf(kwargs['data_file'], quantities=quantities,
                                     level=level)
            print('debug1: ', kwargs['data_file'], quantities, level)
    exit()
    ...
    print("shape = ",data['rho'].shape)
    # {rho[np,nt,nr]}

    
    import numpy as np
    # print(data['rho'][0,0,10])
    with open('ath_test.txt', 'w') as outfile:

        outfile.write('# Array shape: nph, nth, nr: {0}\n'.format(data['rho'].shape))
    ...
    # np.savetxt('rsph.out', r, delimiter=' ')
    # np.savetxt('theta.out', r, delimiter=' ')
